...
from torchvision.datasets import CIFAR10, CIFAR100, ImageFolder, SVHN, LSUN
...
class SVHNOOD():
    def __init__(self, dataroot='', use_gpu=True, num_workers=4, batch_size=128):

        #just ToTensor, nothing else.
        test_transform = test_transforms()

        pin_memory = True if use_gpu else False
        data_root = os.path.join(dataroot, 'cifar10')

        testset = SVHN(root=data_root, transform = test_transform, split="test", download=True)
...
class LSUNOOD():
    def __init__(self, dataroot='', use_gpu=True, num_workers=4, batch_size=128, fixed_version = False):

        #just ToTensor, nothing else.
        test_transform = test_transforms()

        pin_memory = True if use_gpu else False
        version = "LSUN_resize" if not fixed_version else "LSUN_FIX"
        data_root = os.path.join(dataroot, version)
        print("OOD Dataset:", version)
        testset = ImageFolder(data_root, transform=test_transform)
...
class ImageNetOOD():
    def __init__(self, dataroot='', use_gpu=True, num_workers=4, batch_size=128, fixed_version = False):
        ...
        #Taken from CSI code base.
        test_transform = test_transforms()

        pin_memory = True if use_gpu else False
        version = "Imagenet_resize" if not fixed_version else "ImageNet_FIX"
        data_root = os.path.join(dataroot, version)
        print("OOD Dataset:", version)
        testset = ImageFolder(data_root, transform=test_transform)
...
class CIFAR10D(object):
    def __init__(self, dataroot='', use_gpu=True, num_workers=4, batch_size=128, _transforms='', _eval=False, ks_single=3, sigma_single= 0.5, prob_single = 0.5):

        transforms_list = train_transforms(_transforms, ks=ks_single, sigma= sigma_single, prob = prob_single)
